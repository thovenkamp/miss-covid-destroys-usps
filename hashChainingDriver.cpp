    ...
    else
    {
        ...
        for(int i = 0; i < 40000; i++) // loops through the array
        {
            ...
            if(i % 100 == 99); // 100 elements have been inserted
            {
                ...
                for(int j = 0; j < 100; j++)
                {
                    hashT.searchItem(values[j]); //searches for values generated by random indices from for loop above
                }
                ...
                count++;
                startTimeInsert = chrono::high_resolution_clock::now(); // reset time
            }    
        }
    }
